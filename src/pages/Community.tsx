...
const Community: React.FC = () => {
  ...
  const handleShare = useCallback(async (planId: string) => {
    ...
    if (!sharedPlanIds.has(planId) && !(await isEmailVerified())) {
      ...
      setShareError(t('community.errors.verifyEmailForShare'));
    ...
    if (!plansPublic && !sharedPlanIds.has(planId)) {
      setShareError(t('community.errors.sharingDisabled'));
    ...
    try {
      if (sharedPlanIds.has(planId)) {
        ...
        setPublicFeed(prev => [{
          id: planId,
          userId:          user.uid,
          userDisplayName: user.displayName ?? t('community.defaultDisplayName'),
      ...
      <div className="max-w-[1150px] mx-auto px-4 sm:px-10 py-6 sm:py-10">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-xs font-heading text-accent uppercase tracking-widest">{t('community.eyebrow')}</p>
          </div>
          <h1 className="font-heading text-2xl text-text">{t('community.title')}</h1>
          <p className="text-muted text-sm mt-1">{t('community.subtitle')}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={feedTab === 'following' ? t('community.searchPlaceholder.following') : t('community.searchPlaceholder.all')}
        ...
        /* ══ TAKİP ETTİKLERİM SEKMESİ — Kişi listesi ══ */
          ...
          profilesLoading ? (
            ...
            <div className="bg-surface border border-divider rounded-3xl overflow-hidden">
              <div className="px-4 py-3 border-b border-divider">
                <p className="text-xs font-heading text-muted uppercase tracking-wider">
                  {feedSearch
                    ? t('community.following.countFound', { count: filteredProfiles.length })
                    : t('community.following.countFollowing', { count: filteredProfiles.length })}
                </p>
              </div>
              <div className="divide-y divide-divider">
                {filteredProfiles.length === 0 ? (
                  <div className="px-4 py-8 text-center">
                    <p className="text-sm text-muted">{t('community.following.noMatch', { term: feedSearch })}</p>
                  </div>
                    ...
                    <div key={profile.uid} className="flex items-center gap-3 px-4 py-3 hover:bg-surface-2 transition-colors cursor-pointer" onClick={() => navigate(`/profile/${profile.uid}`)}>
                      ...
                      {isPending ? (
                        <div className="flex items-center gap-1.5 shrink-0" onClick={e => e.stopPropagation()}>
                          <span className="text-[11px] text-muted font-medium">{t('community.following.unfollowConfirm')}</span>
                          <button
                          ...
                          >
                            {t('community.following.unfollowYes')}
                          </button>
                          <button
                        ...
                        >
                          {t('community.following.followingBadge')}
                        </button>
                      )}
                    </div>
                  );
            ...
            <div className="bg-surface border border-dashed border-divider rounded-3xl p-12 text-center">
              ...
              >
                {t('community.following.browseAll')}
              </button>
            </div>
          )
        ...
        /* ══ HERKES SEKMESİ — Plan akışı ══ */
          ...
          <div>
            ...
            {displayedFeed.length > FEED_INITIAL && (
              ...
              >
                {feedShowAll ? (
                  <><ChevronUp size={13} /> {t('community.showLess')}</>
                ) : (
                  <><ChevronDown size={13} /> {t('community.showMore', { count: displayedFeed.length - FEED_INITIAL })}</>
                )}
              </button>
            )}
          </div>
          ...
          <div className="bg-surface border border-dashed border-divider rounded-3xl p-12 text-center">
            {feedSearch ? (
              <>
                ...
                >
                  {t('community.feedEmpty.clearSearch')}
                </button>
              </>
            ) : (
              <>
        ...
        {feedTab === 'all' && !feedLoading && !feedError && plans.length > 0 && (
          <div className="mt-8 bg-surface border border-divider rounded-3xl p-5">
            <p className="text-xs font-heading text-muted uppercase tracking-wider mb-3 flex items-center gap-1.5">
              <Globe size={12} strokeWidth={2.5} /> {t('community.shareSection.title')}
            </p>
              ...
              {plans.slice(0, 5).map(p => (
                <div key={p.id} className="flex items-center justify-between gap-3 py-1.5">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted">{t('community.shareSection.days', { count: p.plan.dailyPlans.length })}</p>
                  </div>
                  <button
                  ...
                  >
                    {savingShare === p.id
                      ...
                      : sharedPlanIds.has(p.id)
                        ? <><Globe size={10} /> {t('community.shareSection.shared')}</>
                        : <><Globe size={10} /> {t('community.shareSection.share')}</>
                    }
                  </button>
                </div>
              ))}
